...
from .model import *
from .dataset_embedding import PairDataset, collate_fn
from . import config
...
if __name__ == '__main__':
    ...
    model = AutoModelForTextEncoding.from_pretrained(os.path.join(config_dir, train_config['model_path']), trust_remote_code=True)
    # 统计参数量
    ...
    # 训练温度
    temperature: float | int = train_config.get("train_temperature", 0.07)
    ...
    try:
        with tqdm(train_loader) as pbar:
            for x, y in pbar:
                """
                WARNING
                此处即使batch_size * n_batches_per_step相同，不同的取值仍会影响效果！
                一个大小为batch_size的batch中有1个正例和batch_size - 1个负例
                n_batches_per_step才是真正的batch size。
                """
                # 一个step的开始，更新学习率
                ...
                with autocast(
                    ...
                    embedding_x = F.normalize(model(x), dim=-1)
                    embedding_y = F.normalize(model(y), dim=-1)
                    scores = torch.matmul(embedding_x, embedding_y.T) / temperature
                    target = torch.arange(scores.shape[0]).to(config.DEVICE)
